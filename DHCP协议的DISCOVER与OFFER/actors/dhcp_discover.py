...
#encoding:UTF-8
import struct
def discover(client_mac, request_ip, xid, magic_cookie):
    temp=""
    ...
    return op+htype+hlen+hops+xid+seconds+flags+ciaddr+yiaddr+siaddr+giaddr+client_mac+client_hardware+server_hostname+boot_file+magic_cookie+option1+option3+request_ip+host_name+vendor_class+parameter_request+option4+pading
